...
class WigProfilerwBed:
    ...
    def capture_regions(self, wig, bed):
        ...
        for chrom in chroms:
            ...
            if self.dir:
                try:
                    bediter=zip(bed[chrom]['start'],bed[chrom]['end'], bed[chrom]['strand'])
                except KeyError:
                    dummy=['' for i in range(len(bed[chrom]['start']))]
                    bediter=zip(bed[chrom]['start'],bed[chrom]['end'], dummy)
            else:
                dummy=['' for i in range(len(bed[chrom]['start']))]
                bediter=zip(bed[chrom]['start'],bed[chrom]['end'], dummy)
    ...
    def estimate_wig_interval(self, wig):
        ...
        for chrom in chroms:
            ...
            a.sort()
            starts = [a[i] for i in range(len(a)) if i%2 == 0]
            ends = [a[i] + 2 for i in range(len(a)) if i%2 == 1]# we need at least two element array to get difference
    ...
    def get_breaks(self, start, end):
        ...
        step = self.step
        n = (end-start) / step + 1
        breaks = [int(round(x)) for x in corelib.linspace(start, end, n)]
...
def opt_validate (optparser):
    ...
    if len(options.wig) > 1:
        if options.label:
            ...
            options.label = [os.path.split(x)[1].rsplit('.wig')[0] for x in options.wig]    
...
def draw_multiple_siteprofiles(sitebreaks, avg_siteprofs, legends):
    ...
    minlen = min([len(sitebreaks), min(list(map(len, trim_siteprofs)))])
    trim_siteprofs = [x[:minlen] for x in trim_siteprofs]
...
def dump(chrom, sites, siteprofs):
    ...
    txt = ''
    for start, end, name, siteprof in zip(starts, ends, names, siteprofs):
        s = list(map(str, siteprof))
...
def catBEDs(BEDlist):
    ...
    cBED = inout.Bed()
    cBED.bed = {}
    IDs = range(len(BEDlist))
    for BED, ID in zip(BEDlist, IDs) :
        chroms = BED.get_chroms()
        for chrom in chroms:
        
            # if the first bed to this chromosome, initialize
            if chrom not in cBED:
...
def main():
    ...
    for wig in options.wig:
        ...
        chrcount=1
        FIRST=True
        avg_siteprofs = [[] for x in Siteslist]
        avg_spcounts = [[] for x in Siteslist]
        fixedStep = False
        
        info( "#%d run sitepro on %s..." %(jobcount, os.path.split(wig)[-1]))
            
        for line in open(wig,'r'):
