...
def main():
    ...
    if options.pf:
        ...
        for one_chr in chroms_GeneT:
            newchrom=one_chr
            one_chr_len=int(chroms_lens[one_chr]) # transform from string to long integer
            chr_wigglelike_dump_list=bw.get(one_chr,0,one_chr_len) 
            if chr_wigglelike_dump_list==None: 
                # if there is no information in the bw file for this chromsome, continue
                continue
            
            for line in chr_wigglelike_dump_list:

                l=(line[0],line[2]) 
                # extract chromsomes positions and data values, which are the first and third column
                ...
                if chrom=='' and chrom!=newchrom:
                    ...
                    if chrom in chroms_GeneT:
                        ...
                        outs = list(map(corelib.mean_col_by_col, list(map(corelib.extend_list_series, profiles['exons'])), [True] * len(profiles['exons'])))
                        avg_me = list(map(operator.itemgetter(0), outs))
                        exoncount = list(map(operator.itemgetter(1), outs))
                        outs = list(map(corelib.mean_col_by_col, list(map(corelib.extend_list_series, profiles['introns'])), [True] * len(profiles['introns'])))
                        avg_mi = list(map(operator.itemgetter(0), outs))
                        introncount = list(map(operator.itemgetter(1), outs))
                        del outs
                        ...
                        if not FIRST:    # if not first chromosome
                            ...
                            outs = list(map(corelib.weight_mean_col_by_col, list(map(lambda x, y: [x, y], avg_metaexon, avg_me)),  list(map(lambda x, y: [x, y], avg_exoncount, exoncount)), [True] * n_metas))
                            avg_metaexon = list(map(operator.itemgetter(0), outs))
                            avg_exoncount = list(map(operator.itemgetter(1), outs))
                            outs = list(map(corelib.weight_mean_col_by_col, list(map(lambda x, y: [x, y], avg_metaintron, avg_mi)), list(map(lambda x, y: [x, y], avg_introncount, introncount)), [True] * n_metas))
                            avg_metaintron = list(map(operator.itemgetter(0), outs))
                            avg_introncount = list(map(operator.itemgetter(1), outs))
    ...
    # do profiling for the last chromosome 
        ...
        if options.gn_groups:       # when multiple gene groups are given
            ...
            for x,y in avg_metaexons, avg_metaexon:
                x.append(y)
            for x,y in avg_metaintrons, avg_metaintron:
                x.append(y)                
...
def opt_validate (optparser):
    ...
    if not options.gdb and not options.bed and not options.wig and not options.length_file:
        ...
        sys.exit(1)
    elif not options.bed and not options.wig:
        error('Either a BED file or a WIG file must be given.')
        sys.exit(1)
    elif not options.length_file:
        error('The length file for the chromosomes must be given.(-l)')
        sys.exit(1)
   
    ##
    # check what inputs are given and determine which modules will operate
    ##
    if not os.path.isfile(options.length_file):
        error("No such chrom length file as '%s'" %options.length_file)
        sys.exit(1)
...
def determine_exon_intron_sizes(GeneT, options):
    ...
    if options.elowers and not options.ilowers:
        ...
        medexonsizes = list(map(lambda x,y: (x+y)/2, options.elowers, options.euppers))
    elif not options.elowers and options.ilowers:
        options.elowers = elowers
        options.euppers = euppers
        medintronsizes  = list(map(lambda x,y: (x+y)/2, options.ilowers, options.iuppers))
    ...
    options.metaexonsizes = list(map(corelib.find_nearest_multiple, list(map(max, medexonsizes, [2*options.pf_res]*n_ranges_e)), [options.pf_res]*n_ranges_e))
    options.metaintronsizes = list(map(corelib.find_nearest_multiple, list(map(max, medintronsizes, [2*options.pf_res]*n_ranges_i)), [options.pf_res]*n_ranges_i))
...
def _get_bgroi(GenomeBGP,GenomeBGS,roi,bg_res=100):
    ...
    chroms=set(GenomeBGP.get_chroms()).intersection(list(roisamp.keys()))
    bgroi={}
    whole=0
    for chrom in chroms:
...
def get_chroms_lens(length_file):
