...
class Bed(object):
    ...
    def sort(self,key='start'):
        ...
        for chrom in chroms:
            ...
            try:
                columns=list(self.bed[chrom].keys())
    ...
    def keys(self):
        ...
        return list(self.bed.keys())
    
    def has_key(self,key):
        """Emulate the built-in has_key() method"""
        
        return key in list(self.bed.keys())
    
    def get_chroms(self):
        """Return the chromosomes"""
        try:
            chroms=list(self.bed.keys())
        except AttributeError:
    ...
    def __to_bed(self):
        ...
        for chrom in chroms:
            ...
            for i in range(0,len(self.bed[chrom][self.__columns[1]])):
    ...
    def filter_by_score(self,lowercutoff,uppercutoff=None):
        ...
        ix=[]
        for chrom in chroms:
            for i in range(len(self.bed[chrom]['score'])):
                if uppercutoff:
...
class GeneTable(object):
    ...
    def get_genelengths(self):
        ...
        for chrom in chroms:
            ...
            for i in range(0,current_gene_num):
                exons=list(zip(self[chrom]['exonStarts'][i],self[chrom]['exonEnds'][i]))
                try:
    ...
    def get_gene_lens(self, chrom=''):
        ...
        if chrom != '' and chrom in chroms: # a specific chromomosome is given
            lengths = list(map(lambda start, end: end - start, self[chrom]['txStart'], self[chrom]['txEnd']))
            return lengths
    ...
    def turn2dict(self):
        ...
        dict={}
        for chrom in chroms:
            for i in range(len(self[chrom]['name'])):
                # only if such a name does not exist
                if self[chrom]['name'][i] not in dict:
    ...
    def savedb(self,Db=None,annotation='refGene',overwrite=False):
        ...
        chroms=list(self.table.keys())
        chroms.sort()
        for chrom in chroms:
            for i in range(0,len(self.table[chrom][other_columns[0]])):
    ...
    def get_exon_intron_lens(self, chrom=''):
        ...
        if chrom =='': 
            ...
            for chrom in self.get_chroms():
                for es,ee in zip(self[chrom]['exonStarts'], self[chrom]['exonEnds']):
                    elengths.extend(array_adder(ee,[-1*x for x in es])[0])
                    ilengths.extend(array_adder(es[1:],[-1*x for x in ee[:-1]])[0])
        else:
    ...
    def get_cat_exon_intron_lens(self, chrom=''):
        ...
        if chrom == '':
            ...
            chroms = self.get_chroms()
            for chrom in chroms:
                 cate += list(map(lambda estarts, eends: sum(map(lambda s, e: e-s, estarts, eends)), self[chrom]['exonStarts'], self[chrom]['exonEnds']))
                 cati += list(map(lambda estarts, eends: sum(map(lambda s, e: s-e, estarts[1:], eends[:-1])), self[chrom]['exonStarts'], self[chrom]['exonEnds']))
...
class Wig:
    ...
    def write(self,fnwig,span=1):
        ...
        for chrom in chroms:
            ...
            for i in range(0,len(self.wig[chrom][0])):
...
class XLS:
    ...
    def read(self,fn,columns,types):
        ...
        for line in open(fn,'r'):
            ...
            if ls[0] not in self.xls:
                self.xls[ls[0]]={}
                for column, type, l in zip(columns[1:],types[1:],ls[1:]):
    ...
    def add(self,line):
        ...
        if type(line)==str or type(line)==str:
            ls=line.strip().split()
        else:
            ls=line
            
        if ls[0] in self.xls:
            for column,tp,l in zip(self.columns[1:],self.types[1:],ls[1:]):
    ...
    def toxls(self):
        ...
        for chrom in self.get_chroms():
            ...
            for i in range(len(self.xls[chrom][self.columns[1]])):
...
class DataFrame:
    ...
    def append_column(self,column,colname=''):
        ...
        if colname:
            ...
            ncols=len(list(self.table.keys()))
        ...
        try:
            ...
            if type(key)==tuple:
                if len(key)==2:
                    ...
                    try:
                        for col,val in zip(self.colnames,value): self.table[col][rowix]=val
                    except TypeError:
...
def filter_genes(gene_table,limits=[2900,3100]):
    ...
    for chrom in chroms:
        new_gt[chrom]={}
        columns=list(gene_table[chrom].keys())
        for col in columns:
            new_gt[chrom][col]=[]
        for i in range(0,len(gene_table[chrom]['txStart'])):
...
def estimate_wig_interval(wig):
    ...
    for chrom in chroms:
        ...
        a.sort()
        starts = [a[i] for i in range(len(a)) if a%2 == 0]
        ends = [a[i] for i in range(len(a)) if a%2 == 1]
...
def select_ChIP_by_score_cutoff(ChIP, scorecutoff = 0, descend = True):
    ...
    YESSCORE = True
    for chrom in chroms:
        YESSCORE = YESSCORE and 'score' in ChIP[chrom]
    if not YESSCORE: return None
    
    newChIP = Bed()
    for chrom in chroms:
        if descend:
            b = [x>= scorecutoff for x in ChIP[chrom]['score']]
        else:
            b = [x<= scorecutoff for x in ChIP[chrom]['score']]
        newChIP.bed = {}
        newChIP.bed[chrom]={}
        for key in list(ChIP[chrom].keys()):
...
def select_ChIP_by_n_peaks(ChIP, n_peaks = 1000, descend=True):
    ...
    for chrom in chroms:
        if descend:
            b = [x>= cutoff for x in ChIP[chrom]['score']]
        else:
            b = [x<= cutoff for x in ChIP[chrom]['score']]
        newChIP.bed = {}
        newChIP.bed[chrom]={}
...
def get_percent_from_BED(bed, percentage, twoside=False):
    ...
    YESSCORE = True
    for chrom in chroms:
        YESSCORE = YESSCORE and 'score' in bed[chrom]
...
def downsample_ChIP(ChIP, interval = 5000):
    ...
    for chrom in chroms:
        ...
        for i, start in zip(range(1,len_this_chrom), ChIP[chrom]['start'][1:]):
...
def test_if_valid_BED( bed ):
    ...
    chroms = bed.get_chroms()
    for chrom in chroms:
        for start, end in zip( bed[chrom]['start'], bed[chrom]['end'] ):
...
def draw_a_chrom_heatmap(wig, chrom, ylim, chrominfo, maxchrom, centromere=None, bot = -1, top = 1, line=True, linecol='cyan'):
    ...
    if centromere:
        ...
        if line: 
            ...
            scaled_wig =  [hdelta* scaling * ((max(min(ylim[1],x), ylim[0]) - ylim[1] - ylim[0])/(ylim[1] - ylim[0])) + med for x in wig[1]]
...
def draw_profile_plot(breaks,avg_upstream,avg_downstream,metagene_breaks,avg_metagene,metacatexon_breaks,avg_metacatexon,metacatintron_breaks,avg_metacatintron,metagene_breaks_lim=[-1000,1000]):
    ...
    downbreaks = [b + metagene_breaks[ -1 ] for b in breaks[ start_down:end_down ]]
...
def draw_exon_intron_profile_plot(metaexon_breaks,avg_metaexon,metaintron_breaks,avg_metaintron,elowers,euppers,ilowers,iuppers):    
    ...
    temp_metaexon = [x for x in avg_metaexon if x]
    temp_metaintron = [x for x in avg_metaintron if x]
    
    # ge the global minimum and global maximum
    ylim = [0, 0]
    if temp_metaexon:
        mins_avg_metaexon = list(map(min_w_nan, temp_metaexon))
        maxs_avg_metaexon = list(map(max_w_nan, temp_metaexon))
    else:
        mins_avg_metaexon = []
        maxs_avg_metaexon = []
    
    if temp_metaintron:
        mins_avg_metaintron = list(map(min_w_nan, temp_metaintron))
        maxs_avg_metaintron = list(map(max_w_nan, temp_metaintron))
    else:
        mins_avg_metaintron = []
        maxs_avg_metaintron = []
...
def draw_exon_intron_profile_plots(metaexon_breaks,avg_metaexons,metaintron_breaks,avg_metaintrons,elowers, euppers, ilowers, iuppers, legends=[]):
    ...
    # ge the global minimum and global maximum
    ylim = [0, 0]
    temp_metaexons = [[y for y in x if y] for x in avg_metaexons]
    temp_metaexons = [x for x in temp_metaexons if x]
    if temp_metaexons:            
        mins_avg_metaexons = list(map(min, list(map(min_row_by_row, temp_metaexons,[True]*len(temp_metaexons)))))
        maxs_avg_metaexons = list(map(max, list(map(max_row_by_row, temp_metaexons,[True]*len(temp_metaexons)))))
    else:
        mins_avg_metaexons = []
        maxs_avg_metaexons = []
    
    temp_metaintrons = [[y for y in x if y] for x in avg_metaintrons]
    temp_metaintrons = [x for x in temp_metaintrons if x]
    if temp_metaintrons:
        mins_avg_metaintrons = list(map(min, list(map(min_row_by_row, temp_metaintrons,[True]*len(temp_metaintrons)))))
        maxs_avg_metaintrons = list(map(max, list(map(max_row_by_row, temp_metaintrons,[True]*len(temp_metaintrons))))) 
    else:
